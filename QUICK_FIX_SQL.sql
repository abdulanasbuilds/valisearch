-- ValiSearch Schema Setup — FIX for "already exists" errors
-- Run this if you get "policy already exists" errors

-- 1. First, drop existing policies (to avoid conflicts)
...
create policy "Users can view own analyses" on public.analysis for select to authenticated using (auth.uid() = user_id);
create policy "Users can insert own analyses" on public.analysis for insert to authenticated with check (auth.uid() = user_id);
create policy "Users can delete own analyses" on public.analysis for delete to authenticated using (auth.uid() = user_id);
...
-- ✅ DONE! All tables, policies, and triggers created/updated.
